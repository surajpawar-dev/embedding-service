...
import com.suraj.embeddingservice.exception.ErrorMessage;
...
public class EmbeddingRequestValidator {
    ...
    public void validateBatchRequest(BatchEmbeddingRequest request) {
        ...
        if (uniqueChunkIds.size() != request.chunkIds().size()) {
            throw new InvalidEmbeddingRequestException(ErrorCode.DUPLICATE_CHUNK_IDS, ErrorMessage.DUPLICATE_CHUNK_IDS);
        }
    }
    ...
    public void validateFetchedChunks(BatchEmbeddingRequest request, List<ChunkResponse> chunks) {
        if (chunks.size() != request.chunkIds().size()) {
            throw new InvalidEmbeddingRequestException(ErrorCode.CHUNK_FETCH_FAILED, ErrorMessage.INCOMPLETE_CHUNK_SET);
        }
    ...
    private void validateChunk(ChunkResponse chunk) {
        if (chunk.content() == null || chunk.content().isBlank()) {
            throw new InvalidEmbeddingRequestException(ErrorCode.CHUNK_CONTENT_EMPTY, ErrorMessage.EMPTY_CHUNK_CONTENT);
        }
        if (chunk.checksum() == null || chunk.checksum().isBlank()) {
            throw new InvalidEmbeddingRequestException(ErrorCode.CHECKSUM_VALIDATION_FAILED,
                    ErrorMessage.EMPTY_CHUNK_CHECKSUM);
        }
    }
}
